...
(function() {
  var Promise, fs, ContentTypes, DocPropsApp, JSZip, SharedStrings, Sheet, Style, XlRels, XlWorkbook, baseXl, opt, tool, xml,
  ...
  Sheet = (function() {
    ...
    Sheet.prototype.set = function(col, row, data) {
      if (data == null || data === '') {
        return;
      }

      if (typeof data === 'number') {
        this.data[row][col].v = data;
      } else {
    ...
    Sheet.prototype.setFormula = function(col, row, formula, answer) {
      // // FIXME: it's not this easy. See calcChain.xml
      // this.data[row][col].v = answer;
      // this.data[row][col].f = formula;
      throw Error("not supported yet");
    };

    /**
     * Set data in a row, given a starting cell position
     * @param {number} startCol (optional) Starting column
     * @param {number} startRow Starting row
     * @param {Array}  data     Data array
     */
    Sheet.prototype.setRow = function(startCol, startRow, data) {
      ...
      startRow = args.pop();
      startCol = args.length ? args.pop() : 1;
      for (var i = 0; i < data.length; i++) {
        this.set(startCol + i, startRow, data[i]);
      }
    };

    /**
     * Set data in a column, given a starting cell position
     * @param {number} startCol Starting column
     * @param {number} startRow (optional) Starting row
     * @param {Array}  data     Data array
     */
    Sheet.prototype.setCol = function(startCol, startRow, data) {
      var args = _.toArray(arguments);
      data = args.pop();
      if (!(data instanceof Array)) {
        data = [data];
      }
      startRow = args.length > 1 ? args.pop() : 1;
      startCol = args.pop();
      for (var i = 0; i < data.length; i++) {
        this.set(startCol, startRow + i, data[i]);
      }
    };
  ...
  /**
   * Create and add a Sheet to this Workbook
   * @param  {string} name Name/title of the Sheet
   * @param  {number} cols Number of columns
   * @param  {number} rows Number of rows
   * @return {Sheet}       Reference to the Sheet which was added to the Workbook.
   */
   ...
   * @return {Promise}           Resolves once file is saved
   */
  Workbook.prototype.save = function(savePath, cb) {
    var self = this;
    return Promise.resolve().then(function(){
      ...
      return fs.writeFileAsync(savePath, buffer);
    }).asCallback(cb);
  };

  /**
   * Generate a Buffer containing this XLSX file
   * @param  {Function} cb Callback function (optional)
   * @return {Promise}     Promise which resolves to a Buffer of the XLSX file
   */
